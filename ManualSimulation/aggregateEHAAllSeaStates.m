function summary = aggregateEHAAllSeaStates()
% aggregateEHAAllSeaStates.m
% Scans results/EHA_allSeaStates/task_*.mat (each written the moment its
...
%
% Calls: none
% Called by: Run_EHA_AllSeaStates.m (automatically); also fine to call
...
if isempty(files)
    summary = table();
    warning('aggregateEHAAllSeaStates:noTasks','No task_*.mat files found yet.');
    return
end
...
expectedLabels = {'EHA_mech','EHA_elec'};
...
for L = expectedLabels
    ...
    fprintf('%-10s expected mechRGP=%.4f expected elecRGP=%.4f\n', L{1}, expMechRGP, expElecRGP);
end
end
